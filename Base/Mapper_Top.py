...
class ImageLoader:
    ...
    def __repr__(self):
        js.console.log("<Images images={}>".format(self.__dir__()))
    ...
    def load(self, name, *args, **kwargs):
        ...
        for ext in self.EXTNS:
            ...
            path = '/../../Share/' + str(Cookies.get("UserID")) + "/" + name + '.' + ext
            js.console.log(path)
            try:
        ...
        if res is not None:
            ...
            raise ValueError('Image ' + name + ' not found'+path)
...
class Ptext:
    def draw(self, text, pos=None,
        ...
        if background is not None:
            tsurf = font.render(text, antialias, _resolvecolor(
                color, 'white'), _resolvecolor(background, 'black'))
        else:
...
class Actor:
    ...
    def _calc_anchor(self):
        ...
        # По каким то причинам идет возврат str а не int
        ow, oh = list(map(int, self._orig_surf.get_size()))
    ...
    def angle(self, angle):
        self._angle = angle
        # По каким то причинам идет возврат str а не int
        w, h = list(map(int, self._orig_surf.get_size()))
    ...
    def image(self, image):
        ...
        self._orig_surf = images.load("images/"+image)
